...
bpm_min = 40
f_interval = 0.4
wlength = 10 #second window length
    ...
    for member in members:
        for session in sessions:
            try:
                ...
                t_start = nirs.p.get_start_time()
                t_stop = nirs.p.get_end_time()

                w_start = t_start
                w_stop = t_start + wlength

                while w_stop < t_stop:
                    nirs_window = nirs.p.segment_time(w_start, w_stop)
                      
                    #check nans
                    perc_nan = PercentageNAN([0, 5])(nirs_window)
...
summary.to_csv(os.path.join(signalqualitydir, dataset, modality, 'summary_fnirs_quality_window.csv'))
